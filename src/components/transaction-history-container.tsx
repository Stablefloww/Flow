...
import emptyStateImage1 from '../assets/Empty State1.svg';
import transactionss from './mockData';
import * as Dialog from "@radix-ui/react-dialog";
import usd from '../assets/usdcoin.svg'
import usdt from '../assets/usdt-svgrepo-com.svg'
...
interface Transactionss {
  ...
  coin_type?: 'USDC' | 'USDT'; // Added for future real-time data
}
...
const TransactionHistoryContainer: React.FC<TransactionHistoryContainerProps> = ({
  ...
  const getCoinDisplay = (transaction: Transactionss) => {
    // For real-time data, use the coin_type from the transaction
    if (transaction.coin_type) {
      return {
        logo: transaction.coin_type === 'USDC' ? usd : usdt,
        alt: `${transaction.coin_type} logo`
      };
    }
    
    // For mock data, randomly select between USDC and USDT
    const isUSDC = Math.random() > 0.5;
    return {
      logo: isUSDC ? usd : usdt,
      alt: `${isUSDC ? 'USDC' : 'USDT'} logo`
    };
  };

  const CoinIcon = ({ transaction, size = 24 }: { transaction: Transactionss; size?: number }) => {
    const { logo, alt } = getCoinDisplay(transaction);
    return (
      <div 
      ...
      >
        <div 
          ...
          style={{ 
            ...
            borderRadius: "50%",
            transform: "scale(1.2)" // Make the background slightly larger
          }}
        />
        <Image 
          src={logo}
                ...
                <Dialog.Trigger asChild>
                  ...
                  >
                    <div className='flex flex-row gap-[1.5rem]'>
                      <CoinIcon transaction={transact} />
                      <div className='grid gap-[0.2rem] w-fit'>
                        <h3 className='font-[500] text-[13px] text-[#9B9B9B]'>{formatDate(transact.date, transact.timestamp)}</h3>
                        <h4 className='text-[15px] font-[500] text-[#121212] w-full whitespace-nowrap'>You received {formatNairaAmount(transact.naira_received)}</h4>
                      </div>
                    </div>
                    <div className='flex flex-col gap-[0.2rem] items-end'>
                      <h4 className='text-[15px] font-[500] text-[#121212] whitespace-nowrap'>+{transact.crypto_value}</h4>
                      <h3
                      ...
                      >
                        {capitalizeFirstLetter(transact.status)}
                      </h3>
                    </div>
                  </div>
                </Dialog.Trigger>
                  ...
                  <Dialog.Content>
                    ...
                    >
                      ...
                      <div className="mt-2 grid lg:w-full gap-[2rem]">
                        <div className='bg-[#F7F7F7] py-[0.5rem] lg:w-full flex flex-row justify-between items-center rounded-2xl'>
                          <div className='grid gap-[0.3rem] pt-[0.3rem] lg:px-[1rem]'>
                            ...
                            <div className='flex gap-[0.5rem] items-center justify-center'>
                              <CoinIcon transaction={transact} />
                              <h3 className='font-[600] text-[18px] lg:text-[20px] text-black whitespace-nowrap'>+{transact.crypto_value}</h3>
                            </div>
                          </div>
                          <h4 className='text-[14px] text-[#7C7C7C] font-[500] pr-[0.5rem] whitespace-nowrap'>You received {formatNairaAmount(transact.naira_received)}</h4>
                        </div>
                        <div className='grid gap-[1rem]'>
                          <div className='flex items-center justify-between'>
                            <p className='font-[500] text-[#7C7C7C] text-[16px]'>Crypto value</p>
                            <div className='flex gap-[0.5rem] items-center justify-center'>
                              <CoinIcon transaction={transact} size={20} />
                              <h3 className='font-[600] text-[16px] text-black whitespace-nowrap'>+{transact.crypto_value}</h3>
                            </div>
                          </div>
                          <div className='flex items-center justify-between'>
                            <p className='font-[500] text-[#7C7C7C] text-[16px]'>Naira received</p>
                            <h3 className='font-[600] text-[16px] text-black whitespace-nowrap'>{formatNairaAmount(transact.naira_received)}</h3>
                          </div>
                          <div className='flex items-center justify-between'>
                          ...
                          <div className='flex items-center justify-between'>
                            ...
                            <h3 className='font-[600] text-[16px] text-black'>{formatDate(transact.date, transact.timestamp)}</h3>
                          </div>
                          <div className='flex items-center justify-between'>
                          ...
                          <div className='flex items-center justify-between'>
                            ...
                            <div className={`flex items-center justify-center font-[500] text-[13px] ${
                              transact.status === "completed" 
